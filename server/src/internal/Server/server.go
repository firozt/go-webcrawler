...
func (s *Server) Run() {
	...
	mux.HandleFunc("/v1/crawl", s.MiddleWare("POST", s.StartCrawl))
	mux.HandleFunc("/v1/search", s.MiddleWare("GET", s.SearchCrawled))
	mux.HandleFunc("/v1/graph", s.MiddleWare("GET", s.GetGraphData))
	mux.HandleFunc("/v1/health", s.MiddleWare("GET", s.Health))
